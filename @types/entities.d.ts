...
export declare type AssetExchange =
  | 'AMEX'
  | 'ARCA'
  | 'BATS'
  | 'NYSE'
...
export declare type OrderStatus =
  ...
  /**
   *
   * These are events that may rarely be sent due to unexpected circumstances on the exchanges.
   * It is unlikely you will need to design your code around them, but you may still wish to account
   * for the possibility that they will occur.
   *
   */
  ...
  /**
   * Sent when the order has been received by Alpaca and routed to the exchanges,
   * but has not yet been accepted for execution.
   */
  | 'pending_new'

  /**
   * Sent when your order has been stopped, and a trade is guaranteed for the order,
   * usually at a stated price or better, but has not yet occurred.
   */
  | 'stopped'

  /**
   * Sent when the order is awaiting cancelation. Most cancelations will occur without
   * the order entering this state.
   */
  | 'pending_cancel'
  ...
  /**
   * Sent when the order has been completed for the day - it is either “filled” or “done_for_day”
   * - but remaining settlement calculations are still pending.
   */
  | 'calculated'
